...
<img width="1280" height="800" alt="arduinounogerber" src="https://github.com/user-attachments/assets/c6d0ed9c-2bc9-45f9-854d-3577e9750b11" />
...
**v1.0.2** Fixes a regression introduced in v1.0.1 that could leave the Outline toggle button disabled after the panel first loaded. The v1.0.1 stale-panel cleanup compared each mounted panel's stored URL against `window.location.href`, which includes fragments (`#L34`, `#diff-...`) and query strings. When GitHub updated the URL fragment during the async sibling-layer fetch (as happens when the user clicks a line number on a Gerber blob page), the cleanup mistook the fragment change for a page change, removed the panel it had just mounted, and let the async `enableStackup` call land on a removed element. The fresh replacement panel never received the no-outline variant, so its Outline button stayed disabled forever. The comparison now uses only the origin and pathname, so fragment and query changes are correctly treated as the same page. A regression test locks in the fix.
