...
// =============================================================================
// Eighteenth pass: real-world edge-case corpus. These fixtures exercise the
// messy conditions clean boards don't: KiCad-style layer naming, a board
// with no outline file, a single-sided board, uppercase extensions, X2
// attributes, and a malformed file that must fail gracefully. Generated by
// test/fixtures/generate-corpus.mjs.
// =============================================================================

import { buildStackup as buildStackupCorpus, stackupSvgs as stackupSvgsCorpus, renderSingleLayer as renderSingleLayerCorpus } from '../src/core/render.js'
import { looksLikeGerberByName as nameCorpus, looksLikeGerberByContent as contentCorpus } from '../src/core/detect.js'

const CORPUS_ROOT = path.join('test', 'fixtures', 'corpus')

function loadCorpusBoard(dir) {
  const full = path.join(CORPUS_ROOT, dir)
  return fs.readdirSync(full)
    .filter((f) => nameCorpus(f))
    .map((f) => ({ filename: f, content: fs.readFileSync(path.join(full, f), 'utf8') }))
    .filter((f) => contentCorpus(f.content))
}

// KiCad-style naming: the F_Cu / B_Cu / Edge_Cuts files must be recognized
// and composited, with the Edge_Cuts file treated as the outline.
{
  const files = loadCorpusBoard('kicad-naming')
  if (files.length < 4) {
    console.error('FAIL corpus/kicad-naming: too few files recognized,', files.length)
    process.exit(1)
  }
  const r = await buildStackupCorpus(files)
  if (!r.stackup) {
    console.error('FAIL corpus/kicad-naming: no stackup produced')
    process.exit(1)
  }
  if (!r.hasOutline) {
    console.error('FAIL corpus/kicad-naming: Edge_Cuts not detected as outline')
    process.exit(1)
  }
  console.log('PASS corpus/kicad-naming: F_Cu/B_Cu/Edge_Cuts composited with outline')
}

// No-outline board: must still composite, reporting hasOutline=false so the
// boundary is computed from features.
{
  const files = loadCorpusBoard('no-outline')
  const r = await buildStackupCorpus(files)
  if (!r.stackup) {
    console.error('FAIL corpus/no-outline: no stackup produced')
    process.exit(1)
  }
  if (r.hasOutline) {
    console.error('FAIL corpus/no-outline: reported an outline where there is none')
    process.exit(1)
  }
  console.log('PASS corpus/no-outline: composites with computed boundary (hasOutline=false)')
}

// Single-sided board: top-only copper plus outline still composites a Top.
{
  const files = loadCorpusBoard('single-sided')
  const r = await buildStackupCorpus(files)
  if (!r.stackup) {
    console.error('FAIL corpus/single-sided: no stackup produced')
    process.exit(1)
  }
  const svgs = stackupSvgsCorpus(r.stackup)
  if (!svgs.top || svgs.top.length < 100) {
    console.error('FAIL corpus/single-sided: top render too small')
    process.exit(1)
  }
  console.log('PASS corpus/single-sided: composites a Top view from one-sided board')
}

// Uppercase extensions: detection must be case-insensitive.
{
  const files = loadCorpusBoard('uppercase-ext')
  if (files.length < 3) {
    console.error('FAIL corpus/uppercase-ext: uppercase extensions not all recognized,', files.length)
    process.exit(1)
  }
  const r = await buildStackupCorpus(files)
  if (!r.stackup) {
    console.error('FAIL corpus/uppercase-ext: no stackup produced')
    process.exit(1)
  }
  console.log('PASS corpus/uppercase-ext: uppercase .GTL/.GBL/.GKO recognized and composited')
}

// X2 attributes: the top layer declares its function via %TF.FileFunction.
// Confirm the file is recognized and composited (attribute parsing itself is
// covered elsewhere; here we just ensure X2 files don't break the pipeline).
{
  const files = loadCorpusBoard('x2-attributes')
  const r = await buildStackupCorpus(files)
  if (!r.stackup) {
    console.error('FAIL corpus/x2-attributes: no stackup produced')
    process.exit(1)
  }
  console.log('PASS corpus/x2-attributes: X2-attributed layers composite cleanly')
}

// Malformed Gerber: must not throw an uncatchable error. Either it renders
// leniently or throws a catchable Error; a hang or crash is the failure.
{
  const broken = fs.readFileSync(path.join(CORPUS_ROOT, 'malformed', 'broken.gtl'), 'utf8')
  let handled = false
  try {
    const svg = await renderSingleLayerCorpus(broken, false)
    handled = typeof svg === 'string'
  } catch (e) {
    handled = e instanceof Error
  }
  if (!handled) {
    console.error('FAIL corpus/malformed: did not handle malformed input gracefully')
    process.exit(1)
  }
  console.log('PASS corpus/malformed: truncated Gerber handled gracefully (no crash)')
}

console.log('All real-world corpus checks passed.')

// =============================================================================
// Nineteenth pass: pull request diff support. On a PR "Files changed" page,
// the extension fetches the changed-file list, filters to Gerber-shaped
// files, and renders a before/after preview per file. We mock the pulls
// API (meta + files) and the raw fetches at the base and head commits.
// =============================================================================

const PR_OWNER = 'example'
const PR_REPO = 'pr-test'
const PR_NUMBER = 7
const PR_BASE_SHA = 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'
const PR_HEAD_SHA = 'bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb'

// Reuse the arduino top-copper fixture for the "before" and a modified copy
// for the "after" so the two revisions differ.
const prBeforeGerber = arduinoTopCopper
const prAfterGerber = arduinoBottomCopper   // any other valid gerber differs

const PR_META = {
  base: { sha: PR_BASE_SHA, ref: 'main' },
  head: { sha: PR_HEAD_SHA, ref: 'feature', repo: { owner: { login: PR_OWNER }, name: PR_REPO } },
}
const PR_FILES = [
  { filename: 'board.gtl', status: 'modified' },
  { filename: 'newlayer.gbl', status: 'added' },
  { filename: 'README.md', status: 'modified' },   // not a gerber, should be ignored
]

const baseRaw = (file) => `https://raw.githubusercontent.com/${PR_OWNER}/${PR_REPO}/${PR_BASE_SHA}/${file}`
const headRaw = (file) => `https://raw.githubusercontent.com/${PR_OWNER}/${PR_REPO}/${PR_HEAD_SHA}/${file}`

const domPr = new JSDOM(html, {
  url: `https://github.com/${PR_OWNER}/${PR_REPO}/pull/${PR_NUMBER}/files`,
  runScripts: 'outside-only',
  pretendToBeVisual: true,
})
domPr.window.fetch = (url) => {
  // PR meta
  if (url === `https://api.github.com/repos/${PR_OWNER}/${PR_REPO}/pulls/${PR_NUMBER}`) {
    return Promise.resolve({
      ok: true, status: 200,
      json: () => Promise.resolve({
        base: { sha: PR_BASE_SHA, ref: 'main', repo: { owner: { login: PR_OWNER }, name: PR_REPO } },
        head: { sha: PR_HEAD_SHA, ref: 'feature', repo: { owner: { login: PR_OWNER }, name: PR_REPO } },
      }),
    })
  }
  // PR files (page 1; returns < 100 so no page 2)
  if (url.startsWith(`https://api.github.com/repos/${PR_OWNER}/${PR_REPO}/pulls/${PR_NUMBER}/files`)) {
    return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve(PR_FILES) })
  }
  // Raw content at base/head
  if (url === baseRaw('board.gtl')) {
    return Promise.resolve({ ok: true, status: 200, text: () => Promise.resolve(prBeforeGerber) })
  }
  if (url === headRaw('board.gtl')) {
    return Promise.resolve({ ok: true, status: 200, text: () => Promise.resolve(prAfterGerber) })
  }
  if (url === headRaw('newlayer.gbl')) {
    return Promise.resolve({ ok: true, status: 200, text: () => Promise.resolve(prAfterGerber) })
  }
  return Promise.reject(new Error('unexpected fetch ' + url))
}
domPr.window.eval(bundle)
await new Promise((r) => setTimeout(r, 6000))

const prContainer = domPr.window.document.querySelector('[data-ghgv-pr="1"]')
if (!prContainer) {
  console.error('FAIL pr: container did not mount on PR files page')
  process.exit(1)
}
console.log('PASS pr: container mounted on PR files page')

// Two gerber files changed (board.gtl modified, newlayer.gbl added);
// README.md must be ignored. So there should be exactly 2 cards.
const prCards = prContainer.querySelectorAll('.ghgv-pr-card')
if (prCards.length !== 2) {
  console.error('FAIL pr: expected 2 board-file cards, got', prCards.length)
  process.exit(1)
}
console.log('PASS pr: 2 board-file cards (README.md correctly ignored)')

// The modified file should have both Before and After cells.
const modCard = Array.from(prCards).find((c) => c.querySelector('.ghgv-pr-card-title')?.textContent.includes('board.gtl'))
const modCells = modCard.querySelectorAll('.ghgv-pr-cell')
const modLabels = Array.from(modCells).map((c) => c.querySelector('.ghgv-pr-cell-label')?.textContent)
if (modCells.length !== 2 || !modLabels.includes('Before') || !modLabels.includes('After')) {
  console.error('FAIL pr: modified file should have Before + After, got', modLabels)
  process.exit(1)
}
console.log('PASS pr: modified file shows Before and After')

// The added file should have only an After cell.
const addCard = Array.from(prCards).find((c) => c.querySelector('.ghgv-pr-card-title')?.textContent.includes('newlayer.gbl'))
const addCells = addCard.querySelectorAll('.ghgv-pr-cell')
const addLabels = Array.from(addCells).map((c) => c.querySelector('.ghgv-pr-cell-label')?.textContent)
if (addCells.length !== 1 || addLabels[0] !== 'After') {
  console.error('FAIL pr: added file should have After only, got', addLabels)
  process.exit(1)
}
console.log('PASS pr: added file shows After only')

// Both Before and After of the modified file should contain rendered SVG.
const modSvgs = modCard.querySelectorAll('.ghgv-pr-stage svg')
if (modSvgs.length !== 2) {
  console.error('FAIL pr: modified file cells did not both render SVG, got', modSvgs.length)
  process.exit(1)
}
